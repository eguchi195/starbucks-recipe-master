 ...
 * タップで付箋がペリッと剥がれて数値が表示される。再タップで貼り直し。
 */
export default function ScratchCell({ value, revealed, onToggle }: Props) {
  return (
    <button
      type="button"
      onClick={onToggle}
    ...
    >
      <motion.span
        ...
        transition={{ duration: 0.2, delay: revealed ? 0.12 : 0 }}
      >
        {value}
      </motion.span>
      ...
      <AnimatePresence>
        {!revealed && (
          <motion.span
            ...
            exit={{ opacity: 0, rotate: -18, x: 16, y: -34, scale: 1.06 }}
